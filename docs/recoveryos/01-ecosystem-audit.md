...
|---|---------|------|---------|-------------------|-----------|-----|------------------|--------|------------------------------|
| 1 | **VRCC MVP** | participant, coach | Signup → intake → BARC-10 → coach match → messaging/sessions; SupportNow crisis | `vrcc.app` `main` → `src/mvp/` (11 files) | wrangler.jsonc targets Worker `virtualrecovery`; **CORRECTION 2026-07-23: the bundle now live at vrcc.app is NOT built from this repo** (see `02-target-architecture.md` §0) — it queries `gfa_ui`-generation tables; strong inference: `grace-harbor-16`. MVP tables hold the real participant data (7/6/2/2 rows) | vrcc.app (until superseded 2026-07-23 06:10) | `public.participants`, `mvp_*`, `barc10_assessments`, `app_users` | DEPLOYED-THEN-SUPERSEDED; data live | KEEP the Gate-19B loop + SupportNow patterns; shell decision pending D-4 |
...
| 7 | **grace-harbor-16 ("Grace Harbor")** | visitor/community | **VERIFIED 2026-07-23 (public clone):** Lovable-generated marketing/community site — 9 pages (Index, About, Services, Community, CommunitySpaces, GraceCompanion, WallsOfHonor, Contact, NotFound); shares "Connection Prevents Crisis" branding with production but **is NOT the app live at vrcc.app** (none of the live bundle's tables/RPCs appear in it); no Brain Atlas / Tapes / Recovering the Mind | `GFAVRCC/grace-harbor-16` (public; last commit 2026-06-30) | Lovable hosting (INFERENCE) | UNVERIFIED | **A FOURTH Supabase project: `kmvlkvfxrjqrfxsljvxl`** — outside the connected account | DORMANT since 2026-06-30 | Consolidate its concepts (Walls of Honor ↔ `public.honor_posts`; Grace Companion UI) into the participant shell; retire the stray Supabase project |
