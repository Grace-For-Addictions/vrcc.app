...
| 6 | **RecoveryResidenceOS** | residence ops? | UNVERIFIED | `Grace-For-Addictions/RecoveryResidenceOS` (pushed 2026-07-21) | UNVERIFIED | UNVERIFIED | UNVERIFIED | UNVERIFIED | Audit next — likely overlaps #5 |
...
- D-3: Grant audit access to `RecoveryResidenceOS`, `grace-harbor-16`, `GFA-ECO` (and confirm `contact-connect-dashboard`, `Late-Night-Recovery-`, `icrco` status) so the map can be completed.
